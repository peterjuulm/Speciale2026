...
Run 8 September 2026 on `arch` and `ubuntu-vm`. Same protocol as [7
September](2026-09-07-empty-class.md): same lab, same three files, same pinned
SDK. The procedure is written there and not repeated here.
...
protocol works as a protocol: whether the description is enough to hit the same
number on another day. The environment has also shifted slightly since
yesterday: `ubuntu-vm` has been given 2 GB of swap, and both machines have been
rebooted or shut down along the way.
...
If other numbers come out, that is a finding, not an error. It means something
in the environment has changed that we have not described, and today's
environment blocks against yesterday's say what changed.

## The run

First, the result directory. `arch` (fish):
...
The source is the same as yesterday and is not written again. This time the
check is saved:
...
all the commands above. apt has its own `dotnet` with SDK 10.0.111 in
`/usr/bin`, and that fails against the pin.
...
**`arch`: the protocol holds over time.** Run 8 September 14:14, one day after
the first run. Same three source hashes, same `csc.dll`, and the same DLL hash
twice: `541bed823d12e42b5e9e6087c9c32a9d6fbaa9a116aae61fc00c808231e74113`,
identical to 7 September.
...
drifted in the environment between the two runs, so the same number came out.

This is the weakest form of reproducibility: same machine, later point in time.
It had to hold before anything else means anything. It holds.
